...
function esclave_MLR(N, R, O, q, s, OF, OS, Capa, p, alpha, beta)
    println("Lancement Resolution PLNE pour le sous problème esclave pour MLR : ", p)
...
function generer_vk(N, R, O, q, s, OF, OS, Capa, P, alpha, beta)
    ...
    for p in 1:P
        objective_value,x,y = esclave_MLR(N, R, O, q, s, OF, OS, Capa, p, alpha, beta)
        ...
        # cred_plus_gamma += objective_value - sum(alpha[o]*x[o] for o in O) - sum(beta[r]*y[r] for r in R)
